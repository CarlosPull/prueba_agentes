...
function clearSession() {
  ...
  matrixUserId.value = ''; matrixVms.value = []; matrixTargets.value = [];
...
async function refresh() {
  ...
  if (admin.value) {
    ...
    if (matrixUserId.value) await refreshUserMatrix();
  }
}
...
const matrixVms = ref<{ id: string; name: string; active: boolean }[]>([]);
const matrixTargets = ref<Target[]>([]);
const userGrantsMap = reactive<Record<string, { canRead: boolean; canWrite: boolean }>>({});

async function refreshUserMatrix() {
  const selectedUser = matrixUserId.value;
  const current = epoch;
  try {
    const data = await api(`/admin/users/${selectedUser}/permissions`);
    if (current !== epoch || selectedUser !== matrixUserId.value) return;
    matrixVms.value = data.vms.filter((v: { active: boolean }) => v.active);
    matrixTargets.value = data.targets.filter((t: Target) => t.active);
    for (const key of Object.keys(userGrantsMap)) {
      if (!matrixTargets.value.some(t => t.id === key)) delete userGrantsMap[key];
    }
    for (const t of matrixTargets.value) {
      if (userGrantsMap[t.id]) continue;
    ...
    if (current === epoch && selectedUser === matrixUserId.value) {
      matrixVms.value = []; matrixTargets.value = [];
      for (const key of Object.keys(userGrantsMap)) delete userGrantsMap[key];
    }
    throw e;
  }
...
async function loadUserMatrix() {
  matrixVms.value = []; matrixTargets.value = [];
  for (const key of Object.keys(userGrantsMap)) delete userGrantsMap[key];
  if (matrixUserId.value) await action(refreshUserMatrix);
}
