    ...
    <main class="main"><header class="topbar"><span>PLATAFORMA / {{ section.toUpperCase() }}</span><span class="session-indicator">Sesión activa</span></header>
      <div class="content"><p v-if="error" class="alert error" role="alert">{{ error }}</p><p v-if="notice" class="alert" role="status">{{ notice }}</p>
        ...
        <template v-if="section === 'solicitudes'"><div class="page-title"><div><p class="eyebrow">SOLICITUDES</p><h1>Tu espacio de trabajo</h1><p class="muted">Describe el objetivo. El agente trabajará en el módulo que selecciones.</p></div><button class="secondary" @click="action(refresh)" :disabled="busy">Actualizar ↻</button></div>
          ...
          <div class="request-grid"><form class="panel" @submit.prevent="submitRequest"><div class="panel-heading"><h2>Nueva solicitud</h2><span class="pill">Agente especializado</span></div><label>Módulo de destino<select v-model="request.targetId" required @change="request.readOnly = true"><option disabled value="">Selecciona un módulo</option><option v-for="t in targets" :key="t.id" :value="t.id">{{ t.name }} · {{ t.vm_name }}{{ t.execution_ready ? '' : ' · En preparación' }}</option></select></label><p v-if="selectedTarget" class="context-line">{{ selectedTarget.vm_name }} <span> / </span> {{ selectedTarget.name }}</p><label>¿Qué necesitas hacer?<textarea v-model="request.prompt" rows="7" required maxlength="20000" placeholder="Por ejemplo: revisa los endpoints de comentarios y explica cómo se relacionan con los posts."></textarea></label><div class="request-options"><label class="checkbox"><input v-model="request.readOnly" type="checkbox" :disabled="!selectedTarget?.can_write"> Solo lectura</label><small>{{ request.prompt.length.toLocaleString('es') }} / 20.000</small></div><p class="hint">{{ request.readOnly ? 'El agente podrá analizar el módulo sin modificar su código.' : 'El agente podrá preparar cambios en una copia independiente. No se publicarán automáticamente.' }}</p><p v-if="selectedTarget && !selectedTarget.execution_ready" class="alert">Este módulo está pendiente de preparación por el administrador.</p><button class="primary" :disabled="busy || !selectedTarget?.execution_ready || !request.prompt.trim()">Enviar solicitud <span>↗</span></button></form>
