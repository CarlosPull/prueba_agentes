#!/usr/bin/env bash
# Verifica que tools/agentes/crear_agente.sh genere agentes válidos.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SCRIPT="$ROOT/tools/agentes/crear_agente.sh"
...
chmod +x "$TEMP_DIR/root/tools/agentes/crear_agente.sh" "$TEMP_DIR/root/tools/agentes/crear_agente.sh"

(
  cd "$TEMP_DIR/root"
  ./tools/agentes/crear_agente.sh dev-sec-test "Especialista en Seguridad de Prueba" "Auditar vulnerabilidades de seguridad" "pi-harness,snyk" >/dev/null
)
