...
```bash
./tools/orquestacion/orquestar.sh "objetivo"                       # entrada normal: clasifica y ejecuta todo el flujo
./tools/orquestacion/orquestar.sh --descomponer "objetivo"        # mostrar requisitos categorizados en JSON sin ejecutar
```

La secuencia interna, disponible también para diagnóstico manual, es:

```bash
./tools/vms/probar_vms.sh                                # 1. diagnosticar SSH a VMs
./tools/orquestacion/preparar_proyecto.sh "objetivo"               # 2. crear carpeta proyecto + SOLICITUD.md
./tools/despacho/validar_y_despachar.sh <rol> <dir> "tarea"    # 3. despacho con bloqueo físico de re-despacho
./tools/despacho/generar_reporte.sh <dir> "tarea"              # 4. consolidar REPORTE_PI.md y la evidencia
```

Utilidades auxiliares:

```bash
./tools/orquestacion/orquestar.sh --clasificar "objetivo"          # mostrar el rol sin ejecutar agentes
./tools/orquestacion/descomponer_requisitos.sh "objetivo"          # dividir en requisitos backend/frontend/generales
./tools/orquestacion/clasificar_tarea.sh "objetivo"                # clasificador determinista directo
./tools/despacho/generar_evidencia_agente.sh <rol> <dir>       # evidencia del agente ejecutado en la VM
...
./tools/vms/provisionar_vm_pi.sh <rol> --con-sudo-interactivo # preparar una VM nueva con Pi, sin agent-runner/OpenCode
./tools/vms/provisionar_vm_pi.sh <rol> --solo-verificar    # auditar la instalación experimental de Pi
./tools/vms/provisionar_vm_pi.sh <rol> --solo-configurar   # crear interactivamente el perfil en vms.json sin usar SSH
./tools/sincronizacion/instalar_actualizacion_git.sh <rol>            # instalar el pull Git automático (una vez por VM)
./tools/sincronizacion/sincronizar_agente.sh <rol>                   # solicitar inmediatamente un pull Git en la VM
./tools/sincronizacion/sincronizar_agente_local.sh <perfil-vm>       # copiar y activar inmediatamente un agente local
./tools/sincronizacion/instalar_monitor_local.sh                     # instalar monitor macOS cada 30 segundos
...
- **VMs de desarrollo**: se declaran únicamente en `vms.json`. Los perfiles backend y frontend eliminados deben recrearse mediante `tools/vms/provisionar_vm_pi.sh` antes de utilizarlos.
- **Node en VMs**: `despachar_vm.sh` inyecta `PATH` con `/home/serveradmin/.nvm/versions/node/v24.19.0/bin`. Si la versión de Node cambia, actualizar la construcción de `REMOTE_CMD` en `tools/despacho/despachar_vm.sh`.
...
1. Crear el perfil con `tools/vms/provisionar_vm_pi.sh`; guardará IP, usuario, workspace, stack, Pi, ruta del agente, rama e intervalo de consulta.
...
3. Ejecutar `./tools/vms/provisionar_vm_pi.sh <perfil> --con-sudo-interactivo`. Se solicitará la contraseña `sudo` de la VM para instalar paquetes del sistema.
4. Comprobar posteriormente con `./tools/vms/provisionar_vm_pi.sh <perfil> --solo-verificar`.
