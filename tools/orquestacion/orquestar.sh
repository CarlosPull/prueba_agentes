...
: > "$analysis_tmp/despachos.ndjson"
index=0
while IFS= read -r group; do
  [ -n "$group" ] || continue
  index=$((index + 1))
  category="$(jq -r '.category' <<< "$group")"; profile="$(jq -r '.profile' <<< "$group")"
  repository="$(jq -r '.repository' <<< "$group")"
  dispatch_id="$(printf '%03d-%s-%s-%s' "$index" "$category" "$profile" "$repository" | tr -c 'A-Za-z0-9._-' '-')"
  jq -c --arg id "$dispatch_id" '. + {dispatch_id: $id}' <<< "$group" >> "$analysis_tmp/despachos.ndjson"
done < "$groups_file"

if [ -s "$analysis_tmp/despachos.ndjson" ]; then
  jq -s '.' "$analysis_tmp/despachos.ndjson" > "$PROJECT_DIR/DESPACHOS.json"
else
  echo "[]" > "$PROJECT_DIR/DESPACHOS.json"
fi

"$ROOT/tools/despacho/generar_reporte.sh" "$PROJECT_DIR" "$TAREA" --inicializar >/dev/null 2>&1 || true
...
while IFS= read -r group; do
  [ -n "$group" ] || continue
  dispatch_id="$(jq -r '.dispatch_id' <<< "$group")"
  category="$(jq -r '.category' <<< "$group")"; profile="$(jq -r '.profile' <<< "$group")"
...
done < <(jq -c '.[]' "$PROJECT_DIR/DESPACHOS.json")
...
"$ROOT/tools/despacho/generar_reporte.sh" "$PROJECT_DIR" "$TAREA" --consolidar
