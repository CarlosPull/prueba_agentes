...
[ -s "$VMS_CONF" ] || {
	echo "Error: no existe la configuración '$VMS_CONF'." >&2
	exit 1
}
...
CONFIGURAR_PERFIL_NUEVO() {
	...
	[[ "$ip_nuevo" =~ ^[A-Za-z0-9.:-]+$ ]] || {
		echo "Error: IP no válida." >&2
		exit 1
	}
	[[ "$user_nuevo" =~ ^[A-Za-z0-9._-]+$ ]] || {
		echo "Error: usuario no válido." >&2
		exit 1
	}

	read -r -p "Origen del proyecto [local/git] (local): " source_mode_nuevo
	source_mode_nuevo="${source_mode_nuevo:-local}"
	[ "$source_mode_nuevo" = "local" ] || [ "$source_mode_nuevo" = "git" ] || {
		echo "Error: origen no válido." >&2
		exit 1
	}
	...
	if [ "$source_mode_nuevo" = "local" ]; then
		...
		[ -d "$repositories_root" ] || {
			echo "Error: no existe el directorio de repositorios '$repositories_root'." >&2
			exit 1
		}
		...
		if [ "${#repositorios_locales[@]}" -gt 0 ]; then
			...
			if [[ "$selection" =~ ^[0-9]+$ ]] && [ "$selection" -ge 1 ] && [ "$selection" -le "${#repositorios_locales[@]}" ]; then
				...
				echo "Error: selección de repositorio no válida." >&2
				exit 1
			fi
		else
		...
		[ -d "$project_local_nuevo" ] || {
			echo "Error: no existe '$project_local_nuevo'." >&2
			exit 1
		}
		...
		[ -n "$project_git_url_nuevo" ] || {
			echo "Error: URL Git obligatoria." >&2
			exit 1
		}
  ...
  ' <<<"$detected_technology" >/dev/null; then
	...
	[ "$stack_nuevo" = "backend" ] || [ "$stack_nuevo" = "frontend" ] || {
		echo "Error: stack no soportado." >&2
		exit 1
	}
	...
	if [ "$stack_nuevo" = "backend" ]; then
		...
		[ "$repository_kind_nuevo" = "core" ] || [ "$repository_kind_nuevo" = "module" ] || {
			echo "Error: tipo backend no soportado." >&2
			exit 1
		}
	else
	...
	[[ "$repository_id_nuevo" =~ ^[A-Za-z0-9._:-]+$ ]] && [[ "$module_nuevo" =~ ^[A-Za-z0-9._:-]+$ ]] || {
		echo "Error: ID de repositorio o módulo no válido." >&2
		exit 1
	}
	...
	if [ -n "${seleccion_agente:-}" ]; then
		if [[ "$seleccion_agente" =~ ^[0-9]+$ ]] && [ "$seleccion_agente" -ge 1 ] && [ "$seleccion_agente" -le "${#agentes_disponibles[@]}" ]; then
			...
			echo "Error: el agente '$seleccion_agente' no existe en skills/." >&2
			exit 1
		fi
	fi
	...
	[ "$agent_update_mode_nuevo" = "git" ] || [ "$agent_update_mode_nuevo" = "local" ] || {
		echo "Error: actualización no válida." >&2
		exit 1
	}
	...
	if [ "$agent_update_mode_nuevo" = "git" ]; then
		...
		[[ "$git_url_nuevo" =~ ^https://github\.com/[A-Za-z0-9._/-]+\.git$ ]] || {
			echo "Error: repositorio de agentes no válido." >&2
			exit 1
		}
		[[ "$git_branch_nuevo" =~ ^[A-Za-z0-9._/-]+$ ]] || {
			echo "Error: rama de agentes no válida." >&2
			exit 1
		}
		case "$poll_nuevo" in 10 | 15 | 20 | 30 | 60) ;; *)
			echo "Error: intervalo no permitido." >&2
			exit 1
			;;
		esac
	fi
	...
	[[ "$node_version_nuevo" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || {
		echo "Error: Node no válido." >&2
		exit 1
	}
	[[ "$pi_version_nuevo" = "latest" || "$pi_version_nuevo" =~ ^[0-9]+\.[0-9]+\.[0-9]+([.-][A-Za-z0-9.-]+)?$ ]] || {
		echo "Error: Pi no válido." >&2
		exit 1
	}
	...
	memory_gateway_url_nuevo="https://192.168.50.30:9443"
	...
	if [ "$source_mode_nuevo" = "local" ]; then
		...
		if [ -s "$PRIVATE_TECH_MEMORY" ]; then
			jq -e '.version == 1 and (.repositories | type == "object")' "$PRIVATE_TECH_MEMORY" >/dev/null || {
				echo "Error: inventario tecnológico privado inválido: $PRIVATE_TECH_MEMORY" >&2
				exit 1
			}
...
for valor in "$ip" "$user" "$workspace" "$stack" "$node_version" "$install_dependencies" "$remote_agent"; do
	[ -n "$valor" ] || {
		echo "Error: configuración incompleta para '$VM_PROFILE' en vms.json." >&2
		exit 1
	}
done
[ "$project_kind" = "core" ] || [ "$project_kind" = "module" ] || [ "$project_kind" = "frontend" ] || {
	echo "Error: el repositorio principal de '$VM_PROFILE' debe declarar kind core, module o frontend." >&2
	exit 1
}
...
[[ "$node_version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || {
	echo "Error: node_version no válido." >&2
	exit 1
}
...
if [ "$stack" = "backend" ]; then
	...
	[ -s "$PAQUETES_BACKEND_LOCAL" ] || {
		echo "Error: falta el instalador de paquetes backend." >&2
		exit 1
	}
fi
...
if [ "$source_mode" = "git" ]; then
	for valor in "$project_git_url" "$project_git_branch"; do
		[ -n "$valor" ] || {
			echo "Error: configuración Git del proyecto incompleta." >&2
			exit 1
		}
	done
else
	...
	if [ "$stack" = "backend" ]; then
		[ -s "$project_local_path/composer.json" ] || {
			echo "Error: '$project_local_path' no contiene composer.json." >&2
			exit 1
		}
		[ "$project_kind" != "core" ] || [ -s "$project_local_path/artisan" ] || {
			echo "Error: el core '$project_local_path' no contiene artisan." >&2
			exit 1
		}
	else
		[ -s "$project_local_path/package.json" ] || {
			echo "Error: el proyecto frontend no contiene package.json." >&2
			exit 1
		}
	fi
fi
...
if [ "$agent_update_mode" = "git" ]; then
	for valor in "$agent_git_url" "$agent_git_branch" "$agent_git_path" "$agent_poll_seconds"; do
		[ -n "$valor" ] || {
			echo "Error: configuración Git del agente incompleta." >&2
			exit 1
		}
	done
else
	[ -s "$ROOT/$local_agent/SKILL.md" ] || {
		echo "Error: el agente local no contiene SKILL.md." >&2
		exit 1
	}
fi
